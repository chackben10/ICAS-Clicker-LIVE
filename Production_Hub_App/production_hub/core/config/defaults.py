...
def build_default_endpoints() -> list[EndpointDefinition]:
    ...
    return [
        ...
        EndpointDefinition("presentation_by_uuid", "Presentation by UUID", "/presentation/{uuid}", [ActionDefinition("propresenter.get_presentation_by_uuid", {"uuid": "{{uuid}}"})], allowed_methods=["GET"], inputs=[EndpointInputDefinition("uuid", "Presentation UUID", "string", required=True)], response=data_response, behavior_mode="read"),
        EndpointDefinition("focused_playlist", "Focused Playlist", "/playlist/focused", [ActionDefinition("propresenter.get_focused_playlist")], allowed_methods=["GET"], response=data_response, behavior_mode="read"),
        EndpointDefinition("playlist_by_uuid", "Playlist by UUID", "/playlist/{uuid}", [ActionDefinition("propresenter.get_playlist_by_uuid", {"uuid": "{{uuid}}"})], allowed_methods=["GET"], inputs=[EndpointInputDefinition("uuid", "Playlist UUID", "string", required=True)], response=data_response, behavior_mode="read"),
        EndpointDefinition("trigger_presentation_slide", "Trigger Presentation Slide", "/presentation/{uuid}/{index:int}/trigger", [ActionDefinition("propresenter.trigger_presentation_slide", {"uuid": "{{uuid}}", "index": "{{index}}"})], allowed_methods=["POST"], inputs=[EndpointInputDefinition("uuid", "Presentation UUID", "string", required=True), EndpointInputDefinition("index", "Slide index", "integer", required=True, min_value="0")], response=data_response),
        EndpointDefinition("trigger_playlist_presentation_slide", "Trigger Playlist Presentation Slide", "/playlist/{playlist_uuid}/{item_index:int}/{slide_index:int}/trigger", [ActionDefinition("propresenter.trigger_playlist_presentation_slide", {"playlist_uuid": "{{playlist_uuid}}", "item_index": "{{item_index}}", "slide_index": "{{slide_index}}"})], allowed_methods=["POST"], inputs=[EndpointInputDefinition("playlist_uuid", "Playlist UUID", "string", required=True), EndpointInputDefinition("item_index", "Playlist item index", "integer", required=True, min_value="0"), EndpointInputDefinition("slide_index", "Slide index", "integer", required=True, min_value="0")], response=data_response),
        ...
        EndpointDefinition(
            "clicker_presentation_activation_get",
        ...
        EndpointDefinition(
            "clicker_presentation_activation_set",
            "Set Clicker Presentation Activation",
            "/clicker-presentation-activation",
            [ActionDefinition("runtime.clicker_presentation_activation", {"enabled": "{{enabled}}"})],
            allowed_methods=["POST"],
            inputs=[EndpointInputDefinition("enabled", "Enabled", "bool", required=True)],
            response=data_response,
        ),
...
def build_default_automations() -> list[AutomationDefinition]:
    return [
        AutomationDefinition(
            ...
            trigger="propresenter.presentation_changed",
            enabled=True,
            interval_seconds=0.75,
            cooldown_seconds=2.5,
            conditions=[
                {
                    "condition_type": "propresenter.active_presentation",
                    "params": {"group_count": "1", "first_group_contains": ":"},
                },
                {
                    "condition_type": "propresenter.current_look",
                    "params": {"look_name": "{{bible_look}}", "matches": True},
                    "negate": True,
                }
            ],
            actions=[ActionDefinition("propresenter.trigger_macro", {"macro_name": "{{bible_macro}}"})],
        ...
        AutomationDefinition(
            ...
            trigger="propresenter.look_changed",
            enabled=True,
        ...
        AutomationDefinition(
            ...
            conditions=[{"condition_type": "always", "params": {}}],
            actions=[ActionDefinition("propresenter.audio_from_slide_label")],
