...
class RemotePageDiscoveryTests(unittest.TestCase):
    def test_root_clicker_has_live_song_library_search_view(self) -> None:
        ...
        self.assertIn('id="playlistControls"', html)
        self.assertIn('id="playlistControlsToggle"', html)
        self.assertIn('id="playlistMenuOverlay"', html)
        self.assertIn('id="switchConfirmationOverlay"', html)
        self.assertIn('role="alertdialog"', html)
        self.assertIn('icas-playlist-controls-visible', html)
        self.assertIn('playlistControls: true', html)
        self.assertIn('/playlist/focused', html)
        self.assertIn('/playlist/${encodeURIComponent(uuid)}', html)
        self.assertIn('item?.type === "presentation"', html)
        self.assertIn('item?.destination === "presentation"', html)
        self.assertIn('item?.is_hidden === false', html)
        self.assertIn('type === "header"', html)
        self.assertIn('type === "placeholder"', html)
        self.assertIn('Placeholder · unavailable', html)
        self.assertNotIn('Headers and placeholders are not selectable', html)
        self.assertIn('.playlist-nav-btn.unavailable', html)
        self.assertIn('grid-column: 1 / -1;', html)
        self.assertIn('header.style.backgroundColor = colors.background', html)
        self.assertIn('foreground: whiteContrast >= blackContrast ? "#fff" : "#000"', html)
        self.assertIn('renderPlaylistMenuItems({ preserveFocus: true })', html)
        self.assertIn('function confirmPresentationSwitch(name, index)', html)
        self.assertIn('source.kind === "playlist"', html)
        self.assertIn('/presentation/${encodeURIComponent(uuid)}/${encodeURIComponent(index)}/trigger', html)
        self.assertIn('Return to Active Presentation', html)
